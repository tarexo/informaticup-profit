...
class Environment_Tests(unittest.TestCase):
    ...
    def test_placing_buildings(self):
        ...
        self.assertFalse(env3.add_building(mine),'mines are not allowed to overlap with deposits')
        env3 = Environment.from_json(filename)
        mine = buildings.Mine((8,2),1)
        self.assertTrue(env3.add_building(mine),'mine should be allowed to be build next to deposits')
    ...
    def test_building_constrains(self):
        ...
        self.assertFalse(env3.add_building(combiner),'combiners are not allowed next to deposits')

        #conveyor to deposit
        env3 = Environment.from_json(filename)
        conveyor = buildings.Combiner((9,1),0)
        self.assertFalse(env3.add_building(conveyor),'conveyors are not allowed next to deposits')

        #factory to deposit
        env3 = Environment.from_json(filename)
        factory = buildings.Combiner((8,2),0)
        self.assertFalse(env3.add_building(factory), 'factories are not allowed next to deposits')
        ...
        self.assertTrue(env3.add_building(factory2), 'factories should be allowed to be build next to a facotry')

        #deposit->mine->conveyor->conveyor
        env3 = Environment.from_json(filename)
        mine = buildings.Mine((8,5),3)
        self.assertTrue(env3.add_building(mine), 'mines should be allowed to be build next to a deposits')
        conv1 = buildings.Conveyor((10,7),0)
        self.assertTrue(env3.add_building(conv1), 'conveyors should be allowed to be build next to a mines')
        conv2 = buildings.Conveyor((13,7),0)
        self.assertTrue(env3.add_building(conv2), 'conveyors should be allowed to be build next to a conveyors')
