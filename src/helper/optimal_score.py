...
from buildings.deposit import Deposit
...
def optimal_score(env):
    ...
    turns = (
        env.turns
    )  # get resource to mine and from mine to factory takes 2 turn minimum
...
def calc_best_score(env, combination, turns, all_resources):
    ...
    if len(combination) == 1:
        return calc_best_score_of_single_product(
            env, combination[0], myturns, resources
        )
    indices = []
